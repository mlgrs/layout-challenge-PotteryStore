$.fn.boom = function(e) {
	...
	for(i = 0; i < num; i++) {
		...
		shape.css({
			top: e.pageY - header__contain.offset().top + 20,
			left: e.pageX - header__contain.offset().left + 40,
			'transform': 'scale(0.'+scale+')',
			'transition': sec + 'ms',
			'fill': colors[randBG]
		});

	header__contain.siblings('.particles').append(shape);
...
$(function() {
	$(document).on('click', 'header__contain', function(e) {
		$(this).boom(e);
	});
